...
int main(void) {
	...
	for (int i = 0; i < personas; i++) {
		...
		while (numeroDeCliente < 0) {
			numeroDeCliente = printf("Error, reingrese su Numero De Cliente: ");
			scanf("%d", &numeroDeCliente);
		}
		printf(
		...
		while (estadoCivil != 's' && estadoCivil != 'S' && estadoCivil != 'c'
				&& estadoCivil != 'C' && estadoCivil != 'v'
				&& estadoCivil != 'V') {
			estadoCivil = printf("Error, reingrese su Estado Civil: ");
			fflush(stdin);
			scanf("%c", &estadoCivil);
		}
		...
		while (temperaturaCorporal < 33) {
			printf("Error, reingrese su tempraturaCorporal");
			scanf("%d", &temperaturaCorporal);
		...
		printf(
				...
				while (genero != 'f' && genero != 'F' && genero != 'm'
						&& genero != 'M' && genero != 'o'
						&& genero != 'O') {
					genero = printf("Error, reingrese su Genero: ");
					fflush(stdin);
					scanf("%c", &genero);
				}

}	//if(numeroDeCliente && estadoCivil == 'v' && edad > "60"){
